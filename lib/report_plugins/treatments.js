...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  function deleteTreatment(event) {
    ...
    var okToDelete = $(this).attr('okToDelete');

    if (okToDelete === "1" || window.confirm(buildConfirmText(data))) {
      $.ajax({
        method: 'DELETE'
  ...
  table.append($('<tr>').css('background','gray')
    ...
    .append($('<th>').css('width','300px').attr('align','left').append(translate('Uuid')))
    .append($('<th>').css('width','300px').attr('align','left').append(translate('created at')))
    .append($('<th>').css('width','300px').attr('align','left').append(translate('sysTime')))
  );
  
  sorteddaystoshow.forEach(function (day) {
    table.append($('<tr>')
      .append($('<td>').attr('colspan','15').css('background','lightgray')
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      var okToDelete = '0';
      if (tr.uuid) {
        if(uuids.has(tr.uuid)){
          extraColor = ' style="background: red;" '
          okToDelete = '1';
        }
        else {
          uuids.add(tr.uuid);
      ...
      table.append($('<tr>').addClass('border_bottom')
        .append($('<td' + extraColor +'>')
          .append($('<img>').addClass('deleteTreatment').css('cursor','pointer').attr('title',translate('Delete record')).attr('src',icon_remove).attr('data',JSON.stringify(tr)).attr('day',day).attr('okToDelete', okToDelete))
          .append('&nbsp;')
        ...
        .append($('<td' + extraColor +'>').append(tr.uuid ? tr.uuid : ''))
        .append($('<td>').append(tr.created_at ? tr.created_at : ''))
        .append($('<td>').append(tr.sysTime ? tr.sysTime : ''))
      );
    }
  });
